# User Guide: Capacity Planning (PK Coordinator)
...
- Requesting quotas from praksis places (hospitals, clinics, institutions)
- Managing quota request status (pending, approved, rejected)
- Monitoring available capacity across study programs
- Tracking quota consumption and availability
- Coordinating with contact persons at praksis places
...
#### Issue: Distributed Quota chart showing unexpected numbers

**Possible Causes:**
1. Multiple overlapping requests stacking
2. Pending vs approved requests confusion
3. Date range misunderstanding

**Solutions:**
1. **Check overlapping requests:** Multiple requests for same study/program with overlapping dates will add together
2. **Distinguish purple (approved) from orange (pending):** Hover over chart to see breakdown
3. **Verify date ranges:** Chart shows one year ahead from today; old quotas won't appear
4. Filter to specific study/program to isolate one set of quotas

#### Issue: Conflict warning when creating request

**Cause:** System detected duplicate or overlapping request

**What it means:**
- You have existing request for same study/program/department
- Date ranges overlap
- Might be intentional (requesting additional capacity)
- Might be error (forgot about previous request)

**Solutions:**
1. **Review existing requests:** Check if previous request already covers this need
2. **If additional capacity needed:** Proceed with submission, but add note explaining it's supplemental
3. **If duplicate:** Cancel and modify existing request instead
4. **If dates wrong:** Adjust start/end dates to avoid overlap

#### Issue: Contact person not responding to request

**Possible Actions:**
1. **Wait:** Allow reasonable time (1-2 weeks)
2. **Follow up via chat:** Send polite reminder
3. **Check contact information:** Verify email is correct
4. **Try alternative contact:** Some institutions have multiple coordinators
5. **Request from different department:** Consider parallel request to backup location
6. **Escalate if urgent:** Contact institution administrator

#### Issue: Search not finding expected results

**Troubleshooting Steps:**
1. **Check spelling:** Ensure search term is spelled correctly
2. **Try partial words:** Use "Univ" instead of "University"
3. **Check active filters:** Filters still apply during search; reset them
4. **Search different fields:** Try praksis place name instead of department
5. **Exit and re-enter search:** Close search mode and reopen
6. **Clear all filters:** Reset everything and search again
...
2. **Contact system administrator:** For technical issues or bugs
3. **Reach out to experienced colleagues:** For workflow questions
4. **Contact praksis place directly:** For request-specific questions
5. **Review training materials:** If available for your institution
